  ...
  >
    <template #body>
        <div class="warning-box">
          ...
          <p>
            Révoquer une permission empêchera explicitement l'utilisateur d'y accéder, 
            même si son rôle ou un groupe lui donne cette permission.
          </p>
        </div>
...
async function revokePermission(perm: any) {
  if (!confirm(`Révoquer la permission "${perm.name}" pour cet utilisateur ?\n\nCette action empêchera explicitement l'accès à cette permission.`)) {
    return;
  }
  ...
  try {
    ...
    alert(error.response?.data?.message || 'Erreur lors de la révocation de la permission');
  }
}
...
.permission-item.revoke:hover .action-icon {
  background: #dc2626;
  color: white;
}
</style>
